...
*   `user_stats`: Aggregates player statistics (games played, won, streaks, etc.). **Crucially, `leaderboard_summary` has a foreign key to `user_stats.player_id`.**
...
1.  **Game End (`/api/guess.ts`):**
    ...
    *   **Critical Pre-computation:** Before this, the API calls `updateUserStats`. This function ensures a record for the `player_id` exists in `user_stats` (upserting if new) and updates their aggregate stats. This is vital because the `leaderboard_summary` table has a foreign key dependency on `user_stats(player_id)`.
