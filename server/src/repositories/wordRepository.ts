...
export const getWordOfTheDay = async (): Promise<WordEntry> => {
  try {
    ...
    if (error) {
      logger.error('Error fetching word of the day from Supabase', { error });
      throw Errors.InternalServer('Failed to fetch word of the day from Supabase');
    }

    if (!data) {
      logger.warn('No word found for today in Supabase', { date: today });
      throw Errors.NotFound('No word found for today in Supabase');
    }
